...
const Footer: React.FC = () => {
  ...
  const [guideTab, setGuideTab] = useState<'intro' | 'steps' | 'tips'>('intro');
  ...
  return (
    <>
      ...
      {showGuide && (
        ...
        >
          <div 
            className="bg-brand-blueLight border border-gray-600 rounded-xl w-full max-w-3xl h-[80vh] flex flex-col shadow-2xl relative"
          ...
          >
            ...
            {/* Modal Tabs */}
            <div className="flex border-b border-gray-700 bg-brand-blue/30">
              <button 
                onClick={() => setGuideTab('intro')}
                className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 transition-colors border-b-2 ${guideTab === 'intro' ? 'text-brand-gold border-brand-gold bg-brand-gold/5' : 'text-gray-400 border-transparent hover:text-white'}`}
              >
                <Info size={16} /> Giới thiệu
              </button>
              <button 
                onClick={() => setGuideTab('steps')}
                className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 transition-colors border-b-2 ${guideTab === 'steps' ? 'text-brand-gold border-brand-gold bg-brand-gold/5' : 'text-gray-400 border-transparent hover:text-white'}`}
              >
                <List size={16} /> Quy trình (Steps)
              </button>
              <button 
                onClick={() => setGuideTab('tips')}
                className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-2 transition-colors border-b-2 ${guideTab === 'tips' ? 'text-brand-gold border-brand-gold bg-brand-gold/5' : 'text-gray-400 border-transparent hover:text-white'}`}
              >
